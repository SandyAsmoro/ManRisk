<template>
  <div class="p-6 mx-auto space-y-8 max-w-7xl">
      ...
      <table class="w-full text-sm text-left text-gray-600 whitespace-nowrap">
        <thead class="border-b border-gray-200 bg-gray-50">
          <tr>
            ...
            <th class="px-6 py-4 font-semibold text-gray-900">Tipe</th>
            <th class="px-6 py-4 font-semibold text-gray-900">PIC Utama</th>
            <th class="px-6 py-4 font-semibold text-center text-gray-900">P26</th>
            <th class="px-6 py-4 font-semibold text-center text-gray-900">R26</th>
            <th class="px-6 py-4 font-semibold text-center text-gray-900">Status</th>
            <th class="px-6 py-4 font-semibold text-center text-gray-900">Aksi</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="item in indicators" :key="item?.id" class="transition-colors border-b border-gray-100 hover:bg-gray-50">
            ...
            <td class="px-6 py-4">{{ item?.pic_section || '-' }}</td>
            <td class="px-6 py-4 font-mono font-medium text-center text-blue-600">{{ item?.p26_initial || '-' }}</td>
            <td class="px-6 py-4 font-mono font-medium text-center text-purple-600">{{ item?.r26_initial || '-' }}</td>
            <td class="px-6 py-4 text-center">
              <span v-if="item?.is_active" class="px-2 py-1 text-xs font-semibold text-green-700 bg-green-100 rounded-full">Aktif</span>
            ...
            <td class="px-6 py-4 space-x-3 text-center">
              <button @click="openModal(item)" class="font-medium text-blue-600 transition hover:text-blue-800">Edit</button>
              <button @click="deleteIndicator(item?.id)" class="font-medium text-red-600 transition hover:text-red-800">Hapus</button>
            </td>
          </tr>
          
          <tr v-if="indicators.length === 0">
            <td colspan="8" class="px-6 py-12 font-medium text-center text-gray-400">
    ...
    <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto bg-gray-900 bg-opacity-50 backdrop-blur-sm">
      <div class="w-full max-w-3xl my-8 bg-white shadow-2xl rounded-xl">
        <div class="sticky top-0 z-10 flex items-center justify-between p-6 bg-white border-b border-gray-100 rounded-t-xl">
          <h3 class="text-xl font-bold text-gray-900">{{ form.id ? 'Edit Data Indikator' : 'Tambah Indikator Baru' }}</h3>
        ...
        <form id="indicatorForm" @submit.prevent="saveData" class="p-6 space-y-5 overflow-y-auto max-h-[65vh]">
          ...
          <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
              <label class="block mb-1 text-sm font-semibold text-gray-700">Kode Indikator <span class="text-red-500">*</span></label>
              <input v-model="form.indicator_code" type="text" maxlength="20" class="w-full px-3 py-2 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-500" placeholder="Cth: IKU-01" required />
            </div>
            <div>
              <label class="block mb-1 text-sm font-semibold text-gray-700">Tipe Indikator</label>
              <input v-model="form.indicator_type" type="text" class="w-full px-3 py-2 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-500" placeholder="Cth: KRI, KCI, KPI" />
            </div>
          </div>
          
          <div>
            <label class="block mb-1 text-sm font-semibold text-gray-700">Nama Indikator <span class="text-red-500">*</span></label>
            <input v-model="form.indicator_name" type="text" class="w-full px-3 py-2 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-500" required />
          </div>

          <div>
            <label class="block mb-1 text-sm font-semibold text-gray-700">Deskripsi Indikator</label>
            <textarea v-model="form.indicator_description" rows="2" class="w-full px-3 py-2 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-500" placeholder="Penjelasan rinci mengenai indikator ini..."></textarea>
          </div>
          
          <div>
            <label class="block mb-1 text-sm font-semibold text-gray-700">Deskripsi IRU (Indikator Risiko Utama)</label>
            <textarea v-model="form.iru_description" rows="2" class="w-full px-3 py-2 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-500" placeholder="Penjelasan aspek risiko untuk IRU..."></textarea>
          </div>

          <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
              <label class="block mb-1 text-sm font-semibold text-gray-700">PIC Utama (Section)</label>
              <input v-model="form.pic_section" type="text" class="w-full px-3 py-2 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-500" placeholder="Cth: Dept. Keuangan" />
            </div>
            <div>
              <label class="block mb-1 text-sm font-semibold text-gray-700">Secondary PICs</label>
              <input v-model="form.secondary_pics" type="text" class="w-full px-3 py-2 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-500" placeholder="Pisahkan dengan koma jika lebih dari satu" />
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
              ...
              <input v-model="form.p26_initial" type="number" step="1" class="w-full px-3 py-2 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-500" placeholder="Cth: 12" />
            </div>
            <div>
              <label class="block mb-1 text-sm font-semibold text-gray-700">R26 Initial Score (Batas Kritis)</label>
              <input v-model="form.r26_initial" type="number" step="1" class="w-full px-3 py-2 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-500" placeholder="Cth: 20" />
            </div>
          </div>
          ...
          <div class="flex items-center pt-2">
            ...
            <label for="is_active" class="ml-2 text-sm font-medium text-gray-700">Indikator Aktif (Ditampilkan dalam perhitungan)</label>
          </div>

        </form>

        <div class="sticky bottom-0 z-10 flex justify-end gap-3 p-6 bg-white border-t border-gray-100 rounded-b-xl">
          <button type="button" @click="closeModal" class="px-4 py-2 text-sm font-medium text-gray-700 transition bg-gray-100 rounded-lg hover:bg-gray-200">Batal</button>
          <button type="submit" form="indicatorForm" class="px-4 py-2 text-sm font-medium text-white transition bg-blue-600 rounded-lg hover:bg-blue-700">Simpan Data</button>
        </div>
      </div>
    </div>
