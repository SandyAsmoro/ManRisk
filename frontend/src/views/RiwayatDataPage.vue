...
<template>
  <div class="p-6 mx-auto space-y-6 max-w-7xl">
    ...
    <div
      class="flex flex-col items-start justify-between gap-4 p-6 bg-white border border-gray-100 shadow-sm rounded-xl md:flex-row md:items-center">
      <div>
      ...
      <div class="flex items-center w-full gap-3 md:w-auto">
        <select v-model="filterQuarter" @change="fetchAssessments"
          class="w-full p-2 text-sm border border-gray-300 rounded-lg outline-none focus:ring-blue-500 md:w-32">
        ...
        <input type="number" v-model="filterYear" @change="fetchAssessments"
          class="w-full p-2 text-sm border border-gray-300 rounded-lg outline-none focus:ring-blue-500 md:w-24"
          placeholder="Tahun" />
      </div>
    </div>
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            <tr v-else v-for="item in assessments" :key="item.id" class="transition hover:bg-gray-50">
              ...
              <td class="p-4 text-center">
                <span class="px-2 py-1 text-xs font-bold text-gray-700 bg-gray-200 rounded-md">{{ item.quarter }} {{
                  item.year }}</span>
              </td>

              <td class="p-4 text-center">
                <div class="inline-flex items-center justify-center gap-2">
                  <span class="px-2 py-1 text-xs font-bold text-gray-600 bg-white border border-gray-200 rounded">{{
                    item.frequency }}</span>
              ...
              <td class="p-4 text-center">
                <button v-if="userRole === 'admin' || (item.status !== 'verified' && isQuarterEditable(item.quarter))"
                  @click="openEditModal(item)"
                  class="inline-flex items-center justify-center px-3 py-1.5 text-xs font-semibold text-blue-700 transition bg-white border border-blue-200 rounded-lg hover:bg-blue-50 focus:ring-2 focus:ring-blue-100">
                  Edit
                </button>

                <button v-if="userRole === 'admin' || (item.status !== 'verified' && isQuarterEditable(item.quarter))"
                  @click="deleteAssessment(item.id)"
                  class="inline-flex items-center justify-center px-3 py-1.5 ml-2 text-xs font-semibold text-red-700 transition bg-white border border-red-200 rounded-lg hover:bg-red-50 focus:ring-2 focus:ring-red-100">
                  Hapus
                </button>
              </td>
        ...
        <form @submit.prevent="saveEdit" class="space-y-5">
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" min="1" max="5" v-model="editForm.frequency"
            ...
            <div>
              ...
              <input type="number" min="1" max="5" v-model="editForm.impact"
                class="w-full px-4 py-2 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-500"
                required />
            </div>
          </div>
          ...
          <div>
            ...
            <textarea v-model="editForm.mitigation_action" rows="3"
              class="w-full px-4 py-2 text-sm border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-500"
              placeholder="Jelaskan tindakan pencegahan..." required></textarea>
          </div>

          <div class="p-4 border border-gray-200 rounded-lg bg-gray-50">
            <label class="block mb-2 text-sm font-semibold text-gray-700">Bukti Pendukung</label>

            <div v-if="editForm.has_document"
              class="flex items-center justify-between p-2 mb-3 border border-blue-100 rounded-lg bg-blue-50">
              <span class="text-xs text-blue-800 truncate" :title="editForm.document_name">📎 {{ editForm.document_name
                }}</span>
              <button type="button" @click="deleteDocument(editForm.id)"
                class="text-xs font-bold text-red-600 hover:text-red-800 hover:underline">
                Hapus
              </button>
            </div>

            <input type="file" @change="handleFileChange" accept=".pdf,.png,.jpg,.jpeg,.zip"
              class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 file:mr-4 file:py-1 file:px-3 file:rounded file:border-0 file:text-xs file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" />
            <p class="mt-1 text-xs text-gray-500">Pilih file baru untuk mengganti file lama, atau biarkan kosong.</p>
          </div>

          <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
            <button type="button" @click="showModal = false"
              class="px-4 py-2 text-sm font-semibold text-gray-600 transition bg-gray-100 rounded-lg hover:bg-gray-200">Batal</button>
            <button type="submit" :disabled="isSaving"
              class="px-4 py-2 text-sm font-semibold text-white transition bg-blue-600 rounded-lg hover:bg-blue-700 disabled:opacity-50">
...
// Fungsi untuk menghapus Keseluruhan Data Asesmen
const deleteAssessment = async (assessmentId) => {
  const confirmMsg = "Apakah Anda yakin ingin menghapus data asesmen ini secara permanen?\n\nSemua riwayat pengisian dan dokumen bukti fisik yang terlampir di dalamnya akan ikut terhapus.";

  if (confirm(confirmMsg)) {
    try {
      const res = await api.delete(`/risiko/assessments/${assessmentId}`);
      alert(res.data.message);

      // Muat ulang (refresh) data tabel setelah proses hapus berhasil
      fetchAssessments();
    } catch (error) {
      alert(error.response?.data?.message || "Gagal menghapus data asesmen.");
    }
  }
};

// 5. FUNGSI BARU: Fungsi untuk menghapus dokumen via API
