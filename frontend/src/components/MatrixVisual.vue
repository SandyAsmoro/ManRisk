<template>
  <div class="w-full p-6 mx-auto font-sans bg-white">
    ...
    <div
      class="flex flex-col items-start justify-between gap-3 pb-4 mb-6 border-b sm:flex-row sm:items-center border-gray-50">
      <div>
    ...
    <div v-else class="flex flex-col items-center justify-center w-full gap-10 lg:flex-row lg:items-start"
      :class="{ 'high-contrast-mode': isHighContrast }">

      <div class="mt-2 select-none">

        <!-- 🚨 PERBAIKAN: baris ini sekarang HANYA berisi label sumbu Y + kotak matriks.
       Sebelumnya keduanya satu flex row dengan label sumbu X & "Level Dampak" via
       items-stretch, jadi tinggi kolom label Y ikut ditarik sebesar tinggi
       grid+label X+caption sekaligus -> 5 angka 1-5 dibagi rata ke tinggi yang
       salah, makin ke bawah makin tidak presisi sejajar dgn barisnya. -->
        <div class="relative flex items-stretch">

          <div class="relative flex items-center justify-center w-12 mr-2">
            <div
              class="absolute text-[11px] font-bold text-gray-400 tracking-widest uppercase -rotate-90 whitespace-nowrap mr-10">
              Level Kemungkinan
            </div>
            <div class="grid grid-rows-5 gap-1.5 p-2 h-full text-xs font-bold text-right text-gray-500">
              <span v-for="y in reversedYAxis" :key="'y-label-' + y" class="flex items-center justify-end h-12 sm:h-14">
                {{ y }}
              </span>
            </div>
          </div>

          <div class="grid grid-cols-5 gap-1.5 bg-gray-50 p-2 rounded-xl border border-gray-200 shadow-inner w-fit"
            role="grid" aria-label="Matriks Risiko 5x5">
            <template v-for="freq in reversedYAxis" :key="'row-' + freq">
              <div v-for="impact in xAxis" :key="'cell-' + freq + '-' + impact" role="gridcell"
                ...
                :style="!isHighContrast ? getDynamicStyle(freq, impact) : {}" :aria-label="getAriaLabel(freq, impact)"
                tabindex="0">
                <span class="z-10 text-base font-extrabold tracking-tight" :class="getTextColor(freq, impact)">
                  {{ getCellData(freq, impact).risk_value }}
                </span>
                <span class="absolute bottom-0.5 right-1 text-[9px] opacity-60" :class="getTextColor(freq, impact)"
                  aria-hidden="true">
                  {{ getCellData(freq, impact).symbol }}
                </span>
                <div
                  class="absolute hidden group-hover:block group-focus:block bottom-full mb-2 w-max bg-gray-900 text-white text-[10px] p-2 rounded-md shadow-md z-30 pointer-events-none text-center leading-normal">
        ...
        <!-- Label sumbu X & caption digeser pakai spacer selebar kolom label Y (w-12 + mr-2),
       supaya tetap pas di bawah kotak matriks saja -->
        <div class="flex">
          <div class="w-12 mr-2 shrink-0" aria-hidden="true"></div>
        ...
        <div class="flex">
          <div class="w-12 mr-2 shrink-0" aria-hidden="true"></div>
          <div class="flex-1 text-center mt-2 text-[11px] font-bold text-gray-400 tracking-widest uppercase">
            Level Dampak
          </div>
        </div>
      </div>
      ...
      <div
        class="flex flex-col justify-start w-full pt-6 border-t border-gray-100 lg:w-64 lg:border-t-0 lg:border-l lg:pt-0 lg:pl-8">
        ...
        <div class="flex flex-col gap-3 text-xs" aria-labelledby="legend-title">
          <div v-for="cat in uniqueCategories" :key="cat.category"
            class="flex items-center gap-3 bg-gray-50 p-2.5 rounded-xl border border-gray-100 shadow-sm transition-all hover:bg-gray-100">
            <span class="flex items-center justify-center w-6 h-6 text-xs font-extrabold rounded-lg shadow-sm shrink-0"
            ...
            <div class="flex flex-col">
              ...
              <span class="text-[10px] text-gray-500 font-semibold mt-0.5">Kategori {{ cat.category }} ({{
                cat.mitigation_level }})</span>
            </div>
          </div>
        </div>
      </div>
